...
What this does:
- loads dataset `lidar`
- converts it to a tibble (nicer dataframe)
- adds an ID column to identify rows later
...
What this does:
- takes 80% of the data randomly → training set
- the other 20% (not selected) → test set
- uses `anti_join` so no duplicates
...
What these do:
- `linear_mod` → simple straight line
- `smooth_mod` → smooth curve
- `wiggly_mod` → VERY flexible curve (too wiggly)
...
What this does:
- repeats train/test split 100 times
Each split is stored in `train` and `test`.
